...
__device__ void CheckerboardPropagation(const cudaTextureObject_t *images,
    const cudaTextureObject_t *depths, const Camera *cameras, float4 *plane_hypotheses,
    float *costs, curandState *rand_states, unsigned int *selected_views,
    float4 *prior_planes, unsigned int *plane_masks, const int2 p,
    const PatchMatchParams params, const int iter)
{
    ...
    if (p.y > 2) {
        ...
        costMinPoint = up_far;
        for (int i = 1; i < params.patch_size; ++i)
        {
            if (p.y > 2 + 2 * i)
            {
    ...
    //expf（x）:e的x次方 itr：迭代数：0、1、2 根据不同的迭代次数设置不同的代价阈值
...
__global__ void BlackPixelUpdate(cudaTextureObjects* texture_objects,
    cudaTextureObjects* texture_depths, Camera *cameras, float4 *plane_hypotheses,
    float *costs, curandState *rand_states, unsigned int *selected_views,
    float4 *prior_planes, unsigned int *plane_masks, const PatchMatchParams params, const int iter)
{
...
////////////////////////////////////////////////////////////////////////////
//光场密集匹配
 __global__ void RandomInitializationLF(cudaTextureObjects* texture_objects, float4* plane_hypotheses,
    float2* pcenters, float* costs, curandState* rand_states, unsigned int* selected_views,
    ...
    const int width,const int height)
{
    ...
    else if (params.planar_prior)
    {
        if (plane_masks[center] > 0 && costs[center] >= 0.15f)
        {
            ...
            costs[center] = ComputeMultiViewInitialCostandSelectedViewsLF(texture_objects[0].images, pcenters, p, plane_hypotheses[center], &selected_views[center], params);
            //printf("%f\n", costs[center]);
        }
        else
        {
    ...
    else
    {
        ...
        {
            plane_hypotheses[center] = GenerateRandomPlaneHypothesisLF(p, &rand_states[center],
...
__device__ void CheckerboardPropagationLF(const cudaTextureObject_t* images,
    ...
    const int2 p, const PatchMatchParamsLF params, const int iter,const int WIDTH)
{
    int width = WIDTH;
    int height = WIDTH;
    if (p.x >= width || p.y >= height)
    ...
    if (p.y > 2)
    {
        ...
        costMin = costs[up_far];
        costMinPoint = up_far;
        for (int i = 1; i < params.patch_size; ++i) // 原 i < 11
        {
            if (p.y > 2 + 2 * i)
            {
                int pointTemp = up_far - 2 * i * width;
    ...
    if (p.y < height - 3)
    {
        ...
        costMin = costs[down_far];
        costMinPoint = down_far;
        for (int i = 1; i < params.patch_size; ++i) // 11
        {
            if (p.y < height - 3 - 2 * i)
            {
                int pointTemp = down_far + 2 * i * width;
    ...
    PlaneHypothesisRefinementLF(images, depths,  &plane_hypotheses[center], pcenters,&depth_now, &costs[center], &rand_states[center], view_weights, weight_norm, prior_planes, plane_masks, &restricted_cost, p, params,WIDTH);
}

__device__ void CheckerboardFilterLF( float4 *plane_hypotheses, float *costs, const int2 p,const int WIDTH)
{
    int width = WIDTH;
    int height = WIDTH;
    if (p.x >= width || p.y >= height) {
...
__global__ void BlackPixelFilterLF(float4 *plane_hypotheses, float *costs,const int width)
{
    int2 p = make_int2(blockIdx.x * blockDim.x + threadIdx.x, blockIdx.y * blockDim.y + threadIdx.y);
    if (threadIdx.x % 2 == 0) {
        p.y = p.y * 2;
    ...
    CheckerboardFilterLF( plane_hypotheses, costs, p,width);
}

__global__ void RedPixelFilterLF(float4 *plane_hypotheses, float *costs,const int width)
{
    int2 p = make_int2(blockIdx.x * blockDim.x + threadIdx.x, blockIdx.y * blockDim.y + threadIdx.y);
    if (threadIdx.x % 2 == 0) {
        p.y = p.y * 2 + 1;
...
void LF_ACMP::RunPatchMatchLF()
{
    ...
    RandomInitializationLF<<<grid_size_randinit, block_size_randinit>>>(texture_objects_cuda,
    ...
    //BlackPixelFilterLF<<<grid_size_checkerboard, block_size_checkerboard>>>(plane_hypotheses_cuda, costs_cuda);
    //CUDA_SAFE_CALL(cudaDeviceSynchronize());
    //RedPixelFilterLF<<<grid_size_checkerboard, block_size_checkerboard>>>( plane_hypotheses_cuda, costs_cuda);
    //CUDA_SAFE_CALL(cudaDeviceSynchronize());
...
void LF_ACMP::RunPatchMatchLF_plane()
{
    const int width = images[0].cols;
    const int height = images[0].rows;
    //std::cout << width << " " << height << std::endl;
...
//    BlackPixelFilterLF<<<grid_size_checkerboard, block_size_checkerboard>>>(plane_hypotheses_cuda, costs_cuda,width);
//    CUDA_SAFE_CALL(cudaDeviceSynchronize());
//    RedPixelFilterLF<<<grid_size_checkerboard, block_size_checkerboard>>>( plane_hypotheses_cuda, costs_cuda,width);
//    CUDA_SAFE_CALL(cudaDeviceSynchronize());
