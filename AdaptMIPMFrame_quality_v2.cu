...
namespace LFMVS
{
    ...
    static __device__ __forceinline__ float ComputeBlurWeight_Frame(const float2& blur_value)
    {
        const float diff = blur_value.x - blur_value.y;
        return expf(-0.007368f * diff * diff);
    }
        ...
        float4& disparity_baseline)
    {
        ...
        for (int i = -radius; i <= radius; i += params.radius_increment)
        {
            for (int j = -radius; j <= radius; j += params.radius_increment)
            {
                ...
                // blur 以有效样本均值的形式统计，避免受 patch 可见样本数影响
        ...
        if (valid_count > 0)
        {
            ...
            blur_value.x *= inv_valid;
            blur_value.y *= inv_valid;
        }
        ...
        float4* disp_vector)
    {
        ...
        const float blur_lambda = 0.35f; // 软惩罚，避免 blur 主导光度代价
        ...
        for (int k = 0; k < neigh_count && k < max_neighbors; ++k)
        {
            ...
            const float blur_weight = ComputeBlurWeight_Frame(blur_v);
            c = clamp_cost(c + blur_lambda * (1.0f - blur_weight));
        ...
        float* baseline_norm)
    {
        ...
        // 软接入：轻度偏向更有几何约束的较长基线，但不压制短基线
        for (int k = 0; k < neigh_count && k < max_neighbors; ++k)
        {
            const float tmp = baseline_norm[k] / baseline_max;
            baseline_norm[k] = 0.70f + 0.30f * sqrtf(fmaxf(tmp, 0.0f));
        }
    }
        ...
        const int phase)
    {
        ...
        float baseline_norm[32];
        ComputeBaselineNorm_Frame(centers, neighbor_ids, neighbor_counts, max_neighbors, vid, baseline_norm);
        ...
        for (int i = 0; i < neigh_count; ++i)
        {
            ...
            sampling_probs[i] *= baseline_norm[i];
        }
        ...
        for (int sample = 0; sample < 15; ++sample)
        {
            ...
            for (int image_id = 0; image_id < neigh_count; ++image_id)
            {
                ...
                if (prob > rand_prob)
                {
                    view_weights[image_id] += baseline_norm[image_id];
                    break;
                }
            }
        }
        ...
        if (weight_norm <= 0.0f)
        {
            for (int i = 0; i < neigh_count; ++i)
            {
                view_weights[i] = baseline_norm[i];
                if (neighbor_ids[vid * max_neighbors + i] >= 0)
                    setBit(temp_selected_views, i);
                weight_norm += view_weights[i];
            }
        }
