...
namespace LFMVS
{
    ...
    float MIDisparityFilterCPU::QuantileFromSorted(const std::vector<float>& sorted_vals, float q)
    {
        ...
        float qq = std::max(0.0f, std::min(1.0f, q));
        float pos = qq * static_cast<float>(sorted_vals.size() - 1);
                                                  ...
                                                  float min_valid_disp) const
    {
        ...
        float d00 = d_at(x0, y0);
        float d01 = d_at(x0, y1);
        float d10 = d_at(x1, y0);
        float d11 = d_at(x1, y1);
    ...
    void MIDisparityFilterCPU::DumpMask(const std::string& path, const cv::Mat& reason_mask) const
    {
        ...
        for (int y = 0; y < reason_mask.rows; ++y)
        {
            ...
            {
                unsigned char v = reason_mask.at<unsigned char>(y, x);
                cv::Vec3b c(0, 0, 0);
                if (v == PR_Keep)      c = cv::Vec3b(0, 180, 0);      // green
                if (v == PR_Cost)      c = cv::Vec3b(0, 255, 255);    // yellow
                if (v == PR_NoSupport) c = cv::Vec3b(255, 128, 0);    // blue-ish
                if (v == PR_Cycle)     c = cv::Vec3b(0, 0, 255);      // red
                if (v == PR_Spike)     c = cv::Vec3b(255, 0, 255);    // magenta
    ...
    void MIDisparityFilterCPU::DumpDispErrorGray16PNG(const std::string& path,
                                                      ...
                                                      float scale)
    {
        ...
        cv::Mat out(err_f32.size(), CV_16UC1, cv::Scalar(0));
        for (int y = 0; y < err_f32.rows; ++y)
        {
            const float* ep = err_f32.ptr<float>(y);
            unsigned short* op = out.ptr<unsigned short>(y);
            for (int x = 0; x < err_f32.cols; ++x)
            {
                ...
                float enc = ep[x] * scale + 1.0f; // 1-offset，0 预留给 invalid
                enc = std::max(1.0f, std::min(enc, 65535.0f));
                op[x] = static_cast<unsigned short>(std::round(enc));
            }
        }
        cv::imwrite(path, out);
    }
                                                    ...
                                                    float thresh_px)
    {
        ...
        for (int y = 0; y < err_f32.rows; ++y)
        {
            ...
            for (int x = 0; x < err_f32.cols; ++x)
            {
                ...
                if (ep[x] <= thresh_px)
                    vp[x] = cv::Vec3b(0, 255, 0);   // green
                else
                    vp[x] = cv::Vec3b(0, 0, 255);   // red
            }
        }
        cv::imwrite(path, vis);
    }
                                                                      ...
                                                                      const std::string& unit_text)
    {
        ...
        cv::threshold(clipped, clipped, vmax_px, vmax_px, cv::THRESH_TRUNC);

        cv::Mat img8, color;
        clipped.convertTo(img8, CV_8U, 255.0 / vmax_px, 0.0);
        ...
        {
            uchar v = static_cast<uchar>(std::round(255.0 * (1.0 - static_cast<double>(y) / std::max(1, color.rows - 1))));
        ...
        float t = std::max(0.0f, std::min(thresh_px, vmax_px));
        int y_thresh = static_cast<int>(std::round((1.0f - t / vmax_px) * (color.rows - 1)));
        cv::line(canvas,
        ...
        const std::string& save_root) const
    {
        ...
        cv::Mat disp_err_map(H_, W_, CV_32FC1, cv::Scalar(-1.0f));
        ...
        auto compute_spike_err_from_stage1 = [&](int x, int y) -> float
        {
            const int idx = y * W_ + x;
            const float cur = read_disp(idx);
            if (!std::isfinite(cur) || cur <= cfg.min_valid_disp)
                return std::numeric_limits<float>::quiet_NaN();
            ...
            if ((int)neigh_vals.size() < cfg.spike_min_neighbors)
                return std::numeric_limits<float>::quiet_NaN();

            std::sort(neigh_vals.begin(), neigh_vals.end());
            const float med = QuantileFromSorted(neigh_vals, 0.5f);
            return std::fabs(cur - med);
        };
        ...
        for (int y = 0; y < H_; ++y)
        {
            for (int x = 0; x < W_; ++x)
            {
                ...
                {
                    int lim = std::min((int)in.neigh_keys.size(), cfg.max_triplet);
                    vids.resize(lim);
                    for (int i = 0; i < lim; ++i)
                        vids[i] = i;
                }
                ...
                std::vector<float> geo_errs_px;
                geo_errs_px.reserve(vids.size());

                for (int b : vids)
                {
                    if (b < 0 || b >= (int)in.neigh_keys.size() || b >= (int)in.baselines_px.size())
                        continue;
                    if (b >= (int)in.neigh_gray_f32.size() || in.neigh_gray_f32[b].empty())
                        continue;
                    ...
                    if (std::isfinite(geo_e))
                        geo_errs_px.push_back(geo_e);

                    if (pass_geo && pass_photo)
                        ++good_neighbors;
                }

                // 一致性误差图：至少有足够支持邻域时，记录该像素闭环几何误差的中值
                if ((int)geo_errs_px.size() >= cfg.min_good_neighbors)
                {
                    std::sort(geo_errs_px.begin(), geo_errs_px.end());
                    const float geo_med = QuantileFromSorted(geo_errs_px, 0.5f);
                    disp_err_map.at<float>(y, x) = geo_med;
                }
        ...
        if (cfg.enable_spike_filter)
        {
            for (int y = 0; y < H_; ++y)
            {
                for (int x = 0; x < W_; ++x)
                {
                    ...
                    const float spike_e = compute_spike_err_from_stage1(x, y);
                    if (!std::isfinite(spike_e))
                        continue;

                    float& err_ref = disp_err_map.at<float>(y, x);
                    if (std::isfinite(err_ref) && err_ref >= 0.0f)
                        err_ref = std::max(err_ref, spike_e);
                    else
                        err_ref = spike_e;

                    if (spike_e > cfg.spike_abs_diff)
                    {
                        keep_final[idx] = 0;
        ...
        // ---------- tile stats on consistency error ----------
        std::vector<float> tile_disp_err_vals;
        tile_disp_err_vals.reserve(N / 2);
        int tile_num_le_thresh = 0;

        for (int y = 0; y < H_; ++y)
        {
            for (int x = 0; x < W_; ++x)
            {
                const float e = disp_err_map.at<float>(y, x);
                if (std::isfinite(e) && e >= 0.0f)
                {
                    tile_disp_err_vals.push_back(e);
        ...
        if (!save_root.empty())
        {
            const std::string tile_root = save_root + "/" + frameName + "/" + key->StrRemoveLOD();

            if (cfg.dump_debug_mask)
            {
                const std::string mask_path = tile_root + "/reject_mask.png";
                DumpMask(mask_path, reason_mask);
            }

            if (cfg.dump_disp_error_map)
            {
                DumpDispErrorGray16PNG(tile_root + "/disp_consistency_error_u16.png",
                                       disp_err_map,
                                       cfg.disp_error_png_scale);

                DumpDispErrorHeatMapFixedWithThreshold(tile_root + "/disp_consistency_error_heatmap.png",
                                                       disp_err_map,
                                                       cfg.disp_error_vis_max_px,
                                                       cfg.disp_error_thresh_px,
                                                       "px");

                DumpDispErrorPassMask(tile_root + "/disp_consistency_pass_mask.png",
                                      disp_err_map,
                                      cfg.disp_error_thresh_px);

                DumpDispErrorOverlay(tile_root + "/disp_consistency_overlay.png",
                                     disp_err_map,
                                     cfg.disp_error_thresh_px);
            }
        }

        if (!tile_disp_err_vals.empty())
        {
            std::sort(tile_disp_err_vals.begin(), tile_disp_err_vals.end());

            double sum = 0.0;
            double sqsum = 0.0;
            for (size_t i = 0; i < tile_disp_err_vals.size(); ++i)
            {
                sum += tile_disp_err_vals[i];
                sqsum += static_cast<double>(tile_disp_err_vals[i]) * tile_disp_err_vals[i];
            }

            io_stats.num_disp_error_samples += static_cast<int64_t>(tile_disp_err_vals.size());
            io_stats.num_disp_error_le_thresh += tile_num_le_thresh;
            io_stats.mean_disp_error_px += sum;
            io_stats.rmse_disp_error_px += sqsum;
            io_stats.p90_disp_error_px += QuantileFromSorted(tile_disp_err_vals, 0.9f) *
                                          static_cast<double>(tile_disp_err_vals.size());
        }
    }
        ...
        const std::string& save_root) const
    {
        ...
        for (auto it = problems_map.begin(); it != problems_map.end(); ++it)
        {
        ...
        if (out_stats.num_disp_error_samples > 0)
        {
            out_stats.mean_disp_error_px /= static_cast<double>(out_stats.num_disp_error_samples);
            out_stats.rmse_disp_error_px =
                std::sqrt(out_stats.rmse_disp_error_px / static_cast<double>(out_stats.num_disp_error_samples));
            out_stats.p90_disp_error_px /=
                static_cast<double>(out_stats.num_disp_error_samples);
        }
        ...
        LOG_ERROR("[MIDisparityFilter] frame=", frameName.c_str(),
                  ...
                  " dispErrN=", out_stats.num_disp_error_samples,
                  " dispErrMean=", out_stats.mean_disp_error_px,
                  " dispErrRMSE=", out_stats.rmse_disp_error_px,
                  " dispErrP90=", out_stats.p90_disp_error_px,
                  " dispErr<=T=", out_stats.num_disp_error_le_thresh);
    }
}
