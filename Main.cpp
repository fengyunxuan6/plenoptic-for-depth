...
void ProcessProblem(const std::string &dense_folder, const Problem &problem, bool geom_consistency, bool planar_prior, bool multi_geometrty=false)
{
    ...
    cv::Mat MLA_DisMap_gray= cv::Mat::zeros(height, width, CV_8UC1);
    ...
    for (int col = 0; col < width; ++col)
    {
        for (int row = 0; row < height; ++row)
        {
            ...
            MLA_DisMap_gray.at<uchar>(row,  col) = (plane_hypothesis.w)*255;
        }
    }
    std::string strMLADisPath_key = result_folder + "/depth_color.png";
    cv::Mat disp_color;
    applyColorMap(MLA_DisMap_gray, disp_color, cv::COLORMAP_JET);
    cv::imwrite(strMLADisPath_key, disp_color);
    ...
    cv::imwrite(result_folder+"/depths.tif", depths);
...
void DisparityToDepth(float &Fl,float &bl,float &Bl,cv::Mat & result,cv::Mat & result_d)
{
    for (int i = 0; i < result.cols; ++i)
    {
        for (int j = 0; j < result.rows; ++j)
        {
            if (result.at<float>(i,j)>0.0)
            {
                   result_d.at<float>(i,j)=1.0/(1/Fl-1/(result.at<float>(i,j)*Bl+bl));
            }
        }
    }
}
...
int main(int argc, char** argv)
{
    ...
    try {
        if (argc < 2)
        {
            std::cout << "USAGE: LF dense_folder" << std::endl;
            return -1;
        }
        // 解析命令参数
        CmdLine cmd;
        int type = (int)LFMVS::eIT_LightFieldView;
        std::string strRootPath;
        std::string strSavePath;
        cmd.add(make_option('i', strRootPath, "custom/LightFiled camera data root path"));
        cmd.add(make_option('o', strSavePath, "LightFiled MVS result path"));
        cmd.add(make_option('t', type, "Need MVS Data type"));
        try
        {
            if (argc == 1)
            {
        ...
        {
            std::cout<<"LightFiled root path is EMPTY!"<<std::endl;
            return -1;
        }

        if (type == LFMVS::eIT_LightFieldView)
        {
            try {
                // 设置程序正在退出的标志
                g_program_exiting = false;
                
                // 使用作用域限制DSManager生命周期，确保在return之前正确析构
                {
                    LFMVS::g_Debug_Save = 0;
                    LFMVS::g_Debug_Static = 2; // lzd
                    LFMVS::DepthSolverManager DSManager(strRootPath, strSavePath);
                    LFMVS::eStereoType eStereoType = LFMVS::eST_BlurFeature; // 匹配算法类型   /eST_PlannerPrior
                    DSManager.DepthSolverPipelineBySequence(eStereoType);
                } // DSManager在这里析构
                ...
                try {
                    ...
                    // 忽略CUDA清理可能产生的任何错误
                    std::cout << "Warning: Error during CUDA device reset" << std::endl;
                }
        ...
        else if (type == LFMVS::eIT_CustomMultiView)
        {
            try {
                // 设置程序正在退出的标志
                g_program_exiting = false;
                
                std::vector<LFMVS::Problem> problems;
                LFMVS::GenerateSampleList(strRootPath, problems);

                std::string output_folder = strRootPath + std::string("/ACMP");
                ...
                for (size_t i = 0; i < num_images; ++i)
                {
                    LFMVS::ProcessProblem(strRootPath, problems[i], geom_consistency, planar_prior);
                }
                ...
                for (int geom_iter = 0; geom_iter < geom_iterations; ++geom_iter)
                {
                    ...
                    for (size_t i = 0; i < num_images; ++i)
                    {
                        LFMVS::ProcessProblem(strRootPath, problems[i], geom_consistency, planar_prior, multi_geometry);
                    }
                }
                LFMVS::RunFusion(strRootPath, problems, geom_consistency);
        ...
        std::cerr << "程序执行过程中发生未知异常" << std::endl;
        try {
            cudaDeviceReset();
        } catch (...) {
            // 忽略CUDA清理可能产生的任何错误
