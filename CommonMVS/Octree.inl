...
// S T R U C T S ///////////////////////////////////////////////////
template <typename TYPE, int DIMS, typename DATA_TYPE>
inline TOctreeCellByFusion<TYPE, DIMS, DATA_TYPE>::TOctreeCellByFusion()
    :
    m_child(NULL)
{
} // constructor
template <typename TYPE, int DIMS, typename DATA_TYPE>
inline TOctreeCellByFusion<TYPE, DIMS, DATA_TYPE>::TOctreeCellByFusion(TOctreeCellByFusion* children)
    :
    m_child(children)
{
} // constructor
...
template <typename TYPE, int DIMS, typename DATA_TYPE>
inline void TOctreeCellByFusion<TYPE, DIMS, DATA_TYPE>::Release()
{
    delete[] m_child;
    m_child = NULL;
} // destructor
  // swap the two octrees
template <typename TYPE, int DIMS, typename DATA_TYPE>
inline void TOctreeCellByFusion<TYPE, DIMS, DATA_TYPE>::Swap(TOctreeCellByFusion& rhs)
{
    std::swap(m_child, rhs.m_child);
    uint8_t tmpData[dataSize];
    memcpy(tmpData, m_data, dataSize);
  ...
  // compute item's index corresponding to the containing cell
template <typename TYPE, int DIMS, typename DATA_TYPE>
inline unsigned TOctreeCellByFusion<TYPE, DIMS, DATA_TYPE>::ComputeChild(const POINT_TYPE& item) const
{
    ASSERT(!IsLeaf());
    unsigned idx = 0;
    if (item[0] >= Node().center[0])
...
template <typename TYPE, int DIMS, typename DATA_TYPE>
void TOctreeCellByFusion<TYPE, DIMS, DATA_TYPE>::ComputeCenter(POINT_TYPE centers[])
{
    if (DIMS == 1) {
        centers[0] << -1;
        centers[1] << 1;
  ...
  // count the number of items contained by the given octree-cell
template <typename TYPE, int DIMS, typename DATA_TYPE>
size_t TOctreeCellByFusion<TYPE, DIMS, DATA_TYPE>::GetNumItemsHeld() const
{
    if (IsLeaf())
        return GetNumItems();
    size_t numItems = 0;
