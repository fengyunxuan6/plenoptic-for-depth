...
public:
	template <typename T>
	static T Median(std::vector<T>* elems) {
		ASSERT(!elems->empty());
		const size_t mid_idx = elems->size() / 2;
		std::nth_element(elems->begin(), elems->begin() + mid_idx, elems->end());
		if (elems->size() % 2 == 0) {
			const T mid_element1 = static_cast<T>((*elems)[mid_idx]);
			const T mid_element2 = static_cast<T>(
	...
	static inline bool stringContains(const String& str, const String& sub_str) {
	...
	static String getRuntTimeFolder();
    ...
    static String getFileParentPath(const String& path)
    {
        std::string strTemp = getFilePath(path);
        std::replace(strTemp.begin(), strTemp.end(), '\\', '/');
        std::string::size_type slash = strTemp.find_last_of("/");
        if (slash == strTemp.size() - 1)
        {
            // ȥ�����һ��"/"
            strTemp = strTemp.substr(0, slash);
        }

        std::string::size_type slash_sec = strTemp.find_last_of("/");
        return path.substr(0, slash_sec +1);
    }
    ...
    static String convertFileNameToUnixStyle(const String& fileName)
    {
        std::string new_fileName(fileName);

        std::string::size_type slash = 0;
        while ((slash = new_fileName.find_first_of('\\', slash))
            != std::string::npos)
        {
            new_fileName[slash] = '/';
        }
	...
	//��ȡ������Ϣ
	static String	GetROMInfo();
	static String	GetMaxROMInfo();

	//��ȡϵͳ��Ϣ
	static String	GetCPUInfo();
	static String	GetRAMInfo();
    static size_t	GetRAMSize();
    static size_t	GetAvailRAMSize();
	static int32_t	GetMaxProcessThread(int32_t nNumViews, int32_t width, int32_t height);
	static String	GetOSInfo();
	enum CPUFNC {NA=0, SSE, AVX};
	static const Flags ms_CPUFNC;

	static void		LogBuild();
	static void		LogMemoryInfo(bool bPrint = false);
