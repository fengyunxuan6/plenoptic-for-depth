...
#define wmkdir(x,y) _wmkdir((x))
#define wstat64 _wstati64
...
String SEACAVE::Util::getRuntTimeFolder()
{
#ifdef _MSC_VER
	DWORD retval = 0;
	const DWORD size = MAX_PATH;
	TCHAR path[size];
	retval = GetModuleFileName(GetModuleHandle("IMSJNI.dll"), path, size);

	if (retval != 0 && retval < size)
	{
		String dir(path);
		return getFilePath(ensureUnifySlash(dir));
	}
#endif
	return String();
}

bool SEACAVE::Util::MakeDirectory(const String &path)
{
	if (path.empty())
	{
		return false;
...
/*----------------------------------------------------------------*/
String SEACAVE::Util::GetROMInfo()
{
#if defined(_MSC_VER)
	...
	char Drive[MAX_PATH] = { 0 };//初始化数组用以存储盘符信息
	...
	{
		//char myDrive[2] = { Drive[i],':'};
		String myDrive(&Drive[i]);

		int DType = ::GetDriveType(Drive + i);
			...
			&nTotalNumberOfFreeBytes))//利用GetDiskFreeSpaceEx函数获得指定磁盘信息
		{
			...
			if (i > 0)
				RomInfo += ";";
			RomInfo += String::FormatString("%sTotal-%s,Free-%s", String(myDrive), formatBytes(nTotalNumberOfBytes.QuadPart), formatBytes(nTotalNumberOfFreeBytes.QuadPart));
		}
		i += 4;
	}
	return RomInfo;
...
SEACAVE::String SEACAVE::Util::GetMaxROMInfo()
{
#if defined(_MSC_VER)
	ULARGE_INTEGER nFreeBytesAvailable;
	ULARGE_INTEGER nTotalNumberOfBytes;
	...
	std::vector<std::pair<String, int64_t>> DriveMap;
	std::vector<String> DriveVec;

	int i = 0;
	while (Drive[i - 1] != '\0' || Drive[i] != '\0')//搜索数组尝试得到盘符信息，如果两者皆否，则数组搜索到尽头
	{
		//char myDrive[3] = { Drive[i],':','\\' };
		String strmyDrive(&Drive[i]);
		...
		if (::GetDiskFreeSpaceEx(strmyDrive,
			&nFreeBytesAvailable,
			&nTotalNumberOfBytes,
			&nTotalNumberOfFreeBytes))//利用GetDiskFreeSpaceEx函数获得指定磁盘信息
		{
			int64_t mywholespace = nTotalNumberOfBytes.QuadPart / 1024 / 1024 / 1024;//得到磁盘总空间，单位为G
			int64_t myfreespace = nTotalNumberOfFreeBytes.QuadPart / 1024 / 1024 / 1024;//得到磁盘剩余空间，单位为G

			DriveMap.emplace_back(strmyDrive, myfreespace);
		}
		else
		{
			DriveVec.emplace_back(strmyDrive);
		}
		i += 4;
	}
	std::partial_sort(DriveMap.begin(),
		DriveMap.begin() + DriveMap.size(),
		DriveMap.end(),
		[](const std::pair<String, int64_t>& drive1,
			const std::pair<String, int64_t>& drive2) {
		return drive1.second > drive2.second;
	});

	if(DriveMap.size()>0)
		return  DriveMap[0].first;

	if (DriveVec.size() > 1)
		return DriveVec[1];
	else if(DriveVec.size() ==1)
		return DriveVec[0];
...
int32_t Util::GetMaxProcessThread(int32_t nNumViews, int32_t width, int32_t height)
{
	size_t availRAMSize = Util::GetAvailRAMSize()*0.8f / 1024 / 1024;
	size_t dmapMemSize = width *height * sizeof(float) / 1024 / 1024; // MB
	dmapMemSize = dmapMemSize * (3 + 1 + 1 + 3 + 1) * (1 + nNumViews); // image+image_gray+depth+normal+confidence 参考+邻居

	int32_t nMaxThreads=2;
	if (dmapMemSize != 0)
	{
		int32_t numFile = availRAMSize / dmapMemSize;
		nMaxThreads = std::max(std::min(numFile, 20),2);
	}
	else
	{
		nMaxThreads = 2;
...
#endif // _MSC_VER
    return nTotalPhys + nTotalVirtual;
}

size_t Util::GetAvailRAMSize()
{
#if defined(_MSC_VER)
    ...
    memoryStatus.dwLength = sizeof(memoryStatus);
    ::GlobalMemoryStatusEx(&memoryStatus);
    const size_t nTotalPhys((size_t)memoryStatus.ullAvailPhys);
    const size_t nTotalVirtual((size_t)memoryStatus.ullTotalVirtual);
#else
    MEMORYSTATUS memoryStatus;
    memset(&memoryStatus, sizeof(MEMORYSTATUS), 0);
...
// 注：当前进程下内存使用情况。
// 参数介绍：
// pmc.PageFaultCount: 页访问错误次数
// pmc.PeakWorkingSetSize: 工作集的最大值（即该进程运行以来曾使用过的最大内存数量）！！！
// pmc.WorkingSetSize: 该进程当前工作集（即当前进程正在使用的字节数）！！！
// pmc.QuotaPeakPagedPoolUsage: 使用分页池的最高峰
// pmc.QuotaPagedPoolUsage: 当前使用的分页池的数量
// pmc.QuotaPeakNonPagedPoolUsage: 使用非分页池的最大值
// pmc.QuotaNonPagedPoolUsage: 当前使用非分页池的数量
// pmc.PagefileUsage: 内存管理器已经提交给windows该进程内存的使用值
// pmc.PeakPagefileUsage: 该进程运行以来 提交占用 的最大值
// print information about the memory usage
#ifdef _MSC_VER
#include <Psapi.h>
#pragma comment(lib, "Psapi.lib")
void Util::LogMemoryInfo(bool bPrint/* = false*/)
{
    if (bPrint)
    {
...
#else // _MSC_VER
void Util::LogMemoryInfo(bool bPrint)
{
#if RELWITHDEBINFO
